...
Static landing page for Javo, the AI chat agent for websites. Plain HTML, CSS and JavaScript, no build step.
...
- `styles.css`: styles. Javo tokens, taken from the logo: primary blue `#084efb`, deep navy `#152648`, cyan accent `#05c5fc`, violet `#4e3eff`; Manrope with Instrument Serif italics, pill buttons, 18px cards.
- `main.js`: contact form, messages in the page's language.
- `api/`: Vercel functions that store leads and export them (see Leads).
- `assets/`: demo videos and posters, Javo logo (`javo-logo.png`, `javo-mark.png` for the header), favicons, ALYOR logo.
...
The contact form posts to `api/lead.js`, which stores each lead as a private JSON file in the Vercel Blob store named `omni-leads` (its original name, connected to the project through `BLOB_READ_WRITE_TOKEN`).

- Download all leads as a CSV (opens in Excel or Google Sheets): `https://getjavo.com/api/leads/?key=<LEADS_KEY>`. Add `&format=json` for JSON.
...
- E-mail notification (optional): each new lead is also e-mailed through [Resend](https://resend.com) when these environment variables are set on the Vercel project, followed by a redeploy:
  ...
  - `LEAD_NOTIFY_FROM` (optional): sender, default `Javo <onboarding@resend.dev>`. Resend's default sender can only e-mail the address of the Resend account itself; to e-mail anyone else, verify a sending domain in Resend and use an address on it.
...
This writes `assets/javo-demo-en.mp4` and `assets/javo-demo-fr.mp4` (H.264, 720x1480, 15 s loop) with their posters. The scene text for each language is in `TEXT` in `demo.html`.

## Articles

Articles live in `content/articles.en.mjs` (English) and `content/articles.mjs` (French); both languages must have the same `id`s. Each one opens with a "Calcul Javo" box: a derived number, the formula, the source and a caveat saying what the number is not. Only use figures you can source or derive in the box.
...
- `SITE` at the top of `build.mjs` sets the canonical URLs and the sitemap: `https://getjavo.com`.
...
curl -X POST https://api.indexnow.org/indexnow -H "Content-Type: application/json; charset=utf-8" \
  -d '{"host":"getjavo.com","key":"9654f1c761652d608cb13b9fda3516d3","keyLocation":"https://getjavo.com/9654f1c761652d608cb13b9fda3516d3.txt","urlList":["https://getjavo.com/"]}'
```
